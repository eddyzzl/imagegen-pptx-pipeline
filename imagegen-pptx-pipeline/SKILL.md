...
- **Image quality is enforced before and after ImageGen:** before every single-slide ImageGen call, write the final prompt to `prompts/slide-XXX-comp.txt` and run `scripts/check_imagegen_comp_asset.py --prompt <prompt>`. If the prompt does not explicitly require true 4K `3840x2160`, same dimensions across the deck, highest detail, crisp/no-blur rendering, and at least 5 MiB output, do not call ImageGen. After ImageGen returns, run the same script with `--image slides/slide-XXX-comp.png`; a low-resolution result such as `1672x941` fails immediately and must be regenerated before review.
...
If this fails, regenerate the slide comp at the required resolution before any reviewer or PPTX work.
